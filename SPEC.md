...
- 新規ガイドの作成途中で不要と判断した場合は作成セッションを終了し、その作業候補を手動・全自動のガイド対象から削除できること。削除した候補はガイド作成一覧へ再表示しない。
...
→ 当該会議のクイズに全問正解
→ 推奨作業ごとの骨格・知識台帳を初期化
→ unknown / assumed を「作業前に確認」へ自動降格
→ 準備度ゲート後、confirmed 台帳からガイドを組版
→ 別プロンプトによるAI検収と机上実行レビュー
→ アプリ側スキーマ・台帳突合
→ 「要確認」へ2形式で保存
→ 本人がOK、または指摘だけ入力してAI再修正
```

以下の10ステップは、API設定の有無にかかわらず利用できる手動作成経路として常設する。メインメニューと作業ガイド一覧の両方から開始できること。

```text
STEP 1  作業候補を確認する
STEP 2  現在分かっていることを書き出す
STEP 3  過去資料を検索・選択する（文字起こしは必須で自動選択）
STEP 4  P1で作業の骨格を抽出し、知識台帳を初期化する
STEP 5  P2の取材ループで unknown / assumed を減らす
STEP 6  準備度ゲートを確認し、P3のガイド生成プロンプトを作る
STEP 7  AIから作業ガイドJSONを取得し、スキーマ・台帳突合して取り込む
STEP 8  P4で初見の作業者として机上実行レビューを行い、詰まりを台帳へ還流する
STEP 9  作業ガイド内容を編集・確認する
STEP 10 Google Driveへ保存する
```

作業ガイドは、本番環境のセットアップ手順書と同じ具体度を目標品質とする。目標品質は共通の「品質基準」としてすべてのガイド系プロンプト(P1〜P4・修正・具体度チェック・AI検収)へ埋め込むこと。

- 品質基準: 初めての人が他資料なしで上から実行できる粒度 / 開く画面のURLと画面表記どおりのメニュー・ボタン名 / 入力値の形式・桁数・例と取り違え注意 / 客観的に判定できる完了確認(言い換えだけの完了条件は禁止) / 失敗時に確認する点と戻る手順 / 前提条件・注意事項の網羅 / 判断基準の明示 / 根拠のない値は推測せず「作業前に確認」へ残す。
- 一度の文章生成で品質基準へ到達することは要求しない。文章の書き直しではなく、台帳の不足事実を集める取材を何度でも行えること。
- 知識台帳は `url / ui_label / input_value / verification / failure_recovery / precondition / caution / decision_rule / scope` のスロットと、`confirmed / assumed / unknown` の確度を持つこと。各エントリは出典種別・参照・引用・更新日時を持つこと。
- P2は「回答の理解確認 / 台帳の更新 / 矛盾・気づき / 仮値の監査 / 準備度 / 次の質問」の6セクション固定とする。質問は1ラウンド最大5問、確認式・選択式・現物要求を優先し、同じ質問が2ラウンド残ったら降格候補として示すこと。
- STEP 6ではステップ×スロットのヒートマップを表示する。全必須項目が confirmed または本人が明示的に降格した状態でなければ、ガイド生成へ進めないこと。
- P3は confirmed 台帳だけを事実源としてガイドを組版し、降格項目を `prerequisites` の「作業前に確認」へ必ず出力すること。取り込み時と最終保存時に、URL・主要な具体値・`evidenceRefs`・降格項目を台帳と突合すること。
- P4は生成とは別の初見の作業者ロールで行い、`MISSING_INFO / AMBIGUOUS_CHECK / WRONG_ORDER / RISK_UNFLAGGED / JARGON` の詰まりを実行トレースとともに返すこと。詰まりは台帳の unknown へ還流し、取材または明示的降格後に再生成すること。
- 全自動経路では人へ質問できないため、unknown / assumed と机上実行レビューの詰まりを「作業前に確認」へ自動降格し、残存リスクへ記録すること。
...
- 会議終了から実行可能なガイドの保存までを15分以内で完了できる操作性を目標とする。手動経路では正確な事実確認を優先し、知識台帳の取材往復をJSON生成前に行う。品質基準へ達するための取材・机上実行レビューは15分目標より品質を優先してよい。
...
- 各手順で「ガイド外の資料を見た / 人に聞いた」を1行で記録できること。記録は次バージョンの取材ループの初期質問へ引き継ぐこと。
...
```json
{
  ...
  "verification": {
    "method": "visual | command | value_match",
    "detail": "表示文言・確認コマンドと期待出力・一致すべき値"
  },
  "failureRecovery": {
    "checks": ["失敗時に確認する点"],
    "resumeFrom": "S1"
