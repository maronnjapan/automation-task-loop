...
作業ガイドは、本番環境のセットアップ手順書と同じ具体度を目標品質とする。目標品質は共通の「品質基準」としてすべてのガイド系プロンプト(設計案・深掘り・JSON生成・修正・具体度チェック・AI検収)へ埋め込むこと。
...
- 一度の生成で品質基準へ到達することは要求しない。かわりに、到達するまで生成AIと何度でも往復できる仕組みを提供すること。

- STEP 7 の設計案はJSONを使わず、何をするガイドか、客観的な完了状態、実行順の具体作業と各完了確認、前提条件、注意点、対象外、そして「まだ具体化できていない点と人への質問」を読みやすく表示する。利用者が直接修正でき、明示承認後にだけJSON生成プロンプトを作れる。設計案の変更後は再承認を要求する。
- STEP 7 では、AIの質問への回答・追加情報・指摘を添えて「深掘りプロンプト」を作成できる。深掘りプロンプトは現在の設計案全文と品質基準を含み、改善版設計案の全文を返させる。この往復は回数無制限で、各往復はAI対話履歴(`work_guide_plan_refinement`)へ保存する。深掘り後は再承認が必要になる。
- STEP 8 で取り込んだJSONは、スキーマ検証に加えて具体度チェック(説明の抽象度・作業対象URLの欠落・曖昧な完了確認・前提条件/注意事項の空欄)を行い、不足を保存を妨げない指摘として表示する。
- STEP 9 では、取り込んだドラフトを読みやすいプレビューと具体度チェック結果つきで表示する。「具体度チェックプロンプト」で、品質基準とアプリの指摘を含む点検・修正プロンプトを生成できる。レビュー指摘を入力すると、現在のドラフトJSONと指摘を含む修正プロンプトも生成できる。AIの修正版JSONは STEP 8 へ貼り直して再取り込みし、品質基準へ達するまで納得できるまで往復できる。

- 全自動経路では、初稿と同じ回答を自己承認させず、別の検収プロンプトで初稿の問題・変更内容・残存リスクと修正済みガイドを返させる。
- 全自動経路でも検収後に具体度チェックを行い、不足が残る間は上限回数(設定値 `workGuideDepth.maxAutoRefinements`)まで追加修正をAIへ依頼する。それでも残った不足は「具体度チェック残」として残存リスクへ記録し、人のレビューで確認できるようにする。
...
- 会議終了から実行可能なガイドの保存までを15分以内で完了できる操作性を目標とする。手動経路では正確な認識合わせを優先し、会議確認案とガイド設計案の各往復をJSON生成前に行う。品質基準へ達するための深掘り・具体度チェックの往復は15分目標より品質を優先してよい。
