...
- 生成AIとの連携は OpenAI Responses API を任意設定で利用する。APIキー未設定・停止中・障害時は、プロンプトのコピー&ペースト経路をフォールバックとして残す。
...
別のAI検収工程で根拠・実行可能性・完了条件・安全性を点検し、最終稿へ修正
  ↓
GoogleドキュメントとJSONを「要確認」へ保存
  ↓
本人は理解度クイズへ回答し、作業ガイドを確認
  ↓
...
- 会議解析用プロンプトをGASが生成すること。プロンプトにはAIの出力をJSONコードブロック形式で返すよう指示を含める。画面上のコピー機能はプロンプトのコピーと同時に別タブでChatGPT(chatgpt.com)を開くこと(15分目標の達成手段)。
...
- 作業候補を一覧表示できること。
- 会議解析時に、本人が実行・確認・委譲する具体作業かを `guideRecommended` で判定する。全自動処理では推奨候補を自動でガイド化し、それ以外は候補として残す。
...
以下の11ステップは、API未設定・自動処理失敗・本人が直接編集したい場合の手動フォールバックとして維持する。
...
STEP 6  参照する資料を選択する
STEP 7  AI投入用のコンテキストを作成する(コピー機能で取得しChatGPTを開く)
...
- 上記を支える要件: 5分ごとの自動検出、AI API呼び出し、JSON自動修復、初稿と検収済み最終稿の自動保存。手動フォールバックでは従来のワンタップコピーと即時検証を維持する。
...
- OpenAI APIリクエストは `store: false` とし、アプリ側の監査履歴を正とする。
...
3.  推奨作業のガイド初稿、AI検収、修正版が自動生成される
4.  GoogleドキュメントとJSON、すべてのAI対話がDriveへ保存される
5.  会議内容に応じた数の選択問題へ回答し、途中保存・再開できる
...
OpenAI API設定時は会議解析・クイズ・ガイド初稿・AI検収修正まで自動化し、コピー&ペーストはフォールバックとする
